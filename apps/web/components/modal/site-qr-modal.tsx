...
function SiteQRModalHelper({
	showSiteQRModal,
	setShowSiteQRModal,
	props,
}: {
	showSiteQRModal: boolean;
	setShowSiteQRModal: Dispatch<SetStateAction<boolean>>;
	props: any;
}) {
	return (
		<Modal showModal={showSiteQRModal} setShowModal={setShowSiteQRModal}>
			<QRCodePicker props={props} setShowSiteQRModal={setShowSiteQRModal} />
		</Modal>
	);
}
...
export function QRCodePicker({
	...
	const qrLogoUrl = useMemo(() => {
		...
		return typeof window !== "undefined" && window.location.origin
			...
			: "https://rivvi.io/_static/logo.svg";
	}, [logo, plan]);
		...
		<>
			...
			<div className="flex flex-col space-y-6 bg-gray-50 py-6 text-left sm:rounded-b-2xl">
				...
				<AdvancedSettings
					...
					setShowSiteQRModal={setShowSiteQRModal}
				/>
...
function AdvancedSettings({
	...
	const { setShowAddEditWorkspaceModal, setShowUpgradePlanModal } = useContext(
		ModalContext
	);
						...
						{plan && plan !== "free" ? (
							<div className="mt-1 flex items-center space-x-2">
								...
								<p className="text-sm text-gray-600">
									Show {!workspaceSlug || (!logo && "Phunq.io")} Logo
								</p>
							</div>
						) : (
							<Tooltip
								content={
									<TooltipContent
										...
										cta={
											workspaceSlug ? "Upgrade to Pro" : "Create a workspace"
										}
										{...(APP_HOSTNAMES.has(window.location.hostname)
											? {
													onClick: () => {
														setShowSiteQRModal(false);
														if (workspaceSlug) {
							...
							>
								<div className="pointer-events-none mt-1 flex cursor-not-allowed items-center space-x-2 sm:pointer-events-auto">
									<Switch
										...
										disabled={true}
									/>
									<p className="text-sm text-gray-600">Show Rivvi Logo</p>
								</div>
							</Tooltip>
						)}
					</div>
							...
							download(
								getQRAsSVGDataUri({
									...
									...(showLogo && {
										imageSettings: {
											...qrData.imageSettings,
											src: logo || "https://rivvi.io/_static/logo.svg",
										},
									}),
								}),
								"svg"
...
export function useSiteQRModal({ props }: { props: any }) {
	const [showSiteQRModal, setShowSiteQRModal] = useState(false);

	const SiteQRModal = useCallback(() => {
		return (
			<SiteQRModalHelper
				showSiteQRModal={showSiteQRModal}
				setShowSiteQRModal={setShowSiteQRModal}
				props={props}
			/>
		);
	}, [showSiteQRModal, setShowSiteQRModal, props]);

	return useMemo(() => ({ showSiteQRModal, setShowSiteQRModal, SiteQRModal }), [
		showSiteQRModal,
		setShowSiteQRModal,
		SiteQRModal,
	]);
