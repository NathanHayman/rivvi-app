  ...
  return await Promise.allSettled([
    ...
    ...(conn
      ? [
          key
            ? conn.execute(
                "UPDATE Funnel SET clicks = clicks + 1, lastClicked = NOW() WHERE domain = ? AND `key` = ?",
                [domain, key],
              )
            : conn.execute(
          ...
          conn.execute(
            "UPDATE Workspace w JOIN Domain d ON w.id = d.workspaceId SET w.usage = w.usage + 1 WHERE d.slug = ?",
            [domain],
          ),
        ]
      : []),
