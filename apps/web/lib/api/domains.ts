...
export const validateDomain = async (domain: string) => {
  ...
  const validDomain =
    ...
    // make sure the domain doesn't contain phunq.co/phunq.sh
    !/^(rivvi\.io|.*\.rivvi\.io|rivvi\.co|.*\.rivvi\.co)$/i.test(domain);
...
/* Change the domain for every funnel and its respective stats when the project domain is changed */
export async function changeDomainForSites(domain: string, newDomain: string) {
...
/* Delete a domain and all funnels & images associated with with */
export async function deleteDomainAndSites(
  domain: string,
  {
  ...
  return await Promise.allSettled([
    pipeline.exec(), // delete all sites from redis
