...
export const getFunnelViaEdge = async (domain: string, key: string) => {
  if (!conn) {
    console.error("No connection to PlanetScale");
    return null;
  }

  const { rows } =
    (await conn.execute(
      "SELECT `key`, title, description, image, userId, publicStats FROM Funnel WHERE domain = ? AND `key` = ?",
...
export const getTotalFunnels = async () => {
  if (!cachedConn) return 20000;

  const { rows } = await cachedConn.execute("SELECT COUNT(*) FROM Funnel");
  return rows && Array.isArray(rows) && rows.length > 0
    ? rows[0]["count(*)"]
    : 20000;
};
